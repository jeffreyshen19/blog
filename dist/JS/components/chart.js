...
super(a),this.state={data:[],chart:null,width:0,margin:a.margin,padding:a.padding,height:(a.height||300)-a.margin.top-a.margin.bottom,offset:0,body_width:0,dataset:a},d3.csv(a.csv).then(b=>{this.setState({data:b.map(function(b){return b[a.xcol]=a.parseXCol(b[a.xcol]),a.ycols.split(",").forEach(function(a){b[a]=parseFloat(b[a])}),b})})})}componentWillReceiveProps(a){a!==this.props&&this.renderGraph()}updateDimensions(){//Calculate new width

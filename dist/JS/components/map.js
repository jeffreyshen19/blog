...
d3.svg("/data/police-militarization/us-map-w-puerto-rico.svg").then(a=>{var b=a.documentElement;d3.select(this.state.chart).node().appendChild(b),this.setState({svg:b,data:c})})})}getTooltipText(a,b){function d(a){return("total-cost"==b?"$":"")+parseInt(a).toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}// Mapping from column names to display names

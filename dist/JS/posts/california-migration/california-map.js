...
b.forEach(function(a){c.forEach(function(b){a[b]=parseFloat(a[b])}),a.county=a.county.replace(" County",""),i[a.county]=a}),d3.json("/data/california-migration/california-counties.geojson").then(function(b){function c(){n.attr("d",k)}var d=d3.select("#california-map").select("svg"),j=d3.geoTransform({point:a}),k=d3.geoPath().projection(j),b=b.features.map(function(a){return a.data=i[a.properties.name],a}),l=d3.extent(b,a=>a.data["net-exodus"]),m=d3.scaleDiverging().domain([l[0],0,l[1]]).interpolator(d3.piecewise(d3.interpolateRgb,["#3a539b","#ecf0f1","#c0392b"])),n=d.selectAll("path").data(b).enter().append("path").style("transition","0.1s").style("cursor","pointer").style("pointer-events","visible").attr("stroke","white").attr("fill",function(a){return m(a.data["net-exodus"])}).on("mouseover",function(a){// Change color on hover
d3.select(this).style("fill",d3.rgb(m(.6*a.data["net-exodus"]))),h.html(g(a.data))}).on("mousemove",()=>{var a=d3.mouse(f);h.classed("hidden",!1).style("left",a[0]-Math.round(h.node().offsetWidth/2)+"px").style("top",a[1]+20+"px")}).on("mouseout",function(a){d3.select(this).style("fill",d3.rgb(m(a.data["net-exodus"]))),h.classed("hidden",!0)});// Merge data
e.on("moveend",c),c()})})}getTooltipText(e){function a(a){return parseInt(a).toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}// // Sort county level data into the order they should be displayed

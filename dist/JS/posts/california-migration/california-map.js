...
// Append tooltip
d3.select(this.state.chart).append("div").attr("class","svg").node().appendChild(b),d3.select(this.state.chart).select(".svg").append("div").attr("class","tooltip hidden"),this.setState({svg:b,data:c})})})}getTooltipText(a){return"yo"}renderGraph(){var a=d3.select(this.state.chart).select(".tooltip"),b=this.state.svg,c=this.state.data;// Get radio options
...
d3.select(b).style("width","100%").style("height","100%").attr("viewBox","0 0 900 600").select("#g4864").selectAll("*")}render(){return this.state.data.length&&this.state.svg&&this.renderGraph(),None}}const e=React.createElement;// Render all charts

...
c.forEach(function(a){e.forEach(function(b){a[b]=parseFloat(a[b])}),a.county=a.county.replace(" County",""),f[a.county]=a}),d3.json("/data/california-migration/california-counties.geojson").then(function(c){function d(){k.attr("d",h)}var e=d3.select("#california-map").select("svg"),g=d3.geoTransform({point:a}),h=d3.geoPath().projection(g),c=c.features.map(function(a){return a.data=f[a.properties.name],a}),i=d3.extent(c,a=>a["net-exodus"]),j=d3.scaleDiverging().domain([i[0],0,i[1]]).interpolator(d3.piecewise(d3.interpolateRgb,["#3a539b","#ecf0f1","#c0392b"])),k=e.selectAll("path").data(c.features).enter().append("path").attr("stroke","gray").attr("fill-opacity",.6).style("fill",function(){// return colors(d["net-exodus"]);
return"green"});// Merge data
// // Display SVG
// d3.select(svg)
//   .style("width", "100%")
//   .style("margin", "0 auto")

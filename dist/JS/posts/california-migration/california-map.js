...
// Append tooltip
d3.select(this.state.chart).attr("class","california-map").append("div").attr("class","svg").style("position","relative").style("text-align","center").node().appendChild(b),d3.select(this.state.chart).append("div").attr("class","tooltip hidden"),this.setState({svg:b,data:c})})})}getTooltipText(e){function a(a){return parseInt(a).toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")}// // Sort county level data into the order they should be displayed
...
d3.select(this).style("fill",d3.rgb(h(.6*f(b)))),a.html(e(b))}).on("mousemove",()=>{var b=d3.mouse(this.state.chart);a.classed("hidden",!1).style("left",b[0]+20+"px").style("top",b[1]+20+"px")}).on("mouseout",function(b){d3.select(this).style("fill",d3.rgb(h(f(b)))),a.classed("hidden",!0)})}render(){return this.state.data.length&&this.state.svg&&this.renderGraph(),null}}const e=React.createElement;// Render all charts
